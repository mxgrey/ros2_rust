...
fn main() {
    ...
    let msg_list = if let Some(cmake_includes) = env::var("CMAKE_INCLUDE_DIRS").ok() {
        ...
        let msgs = rclrs_common::get_ros_msgs(&packages);
        rclrs_common::parse_msgs(&msgs)
            .into_iter()
        ...
        let msgs = rclrs_common::get_ros_msgs(&paths);
        rclrs_common::parse_msgs(&msgs)
    };

    let msg_map = rclrs_common::as_map(&msg_list);
    ...
    builder = builder
        ...
        .derive_copy(false)
        // blocklist types that are handled by rcl bindings
