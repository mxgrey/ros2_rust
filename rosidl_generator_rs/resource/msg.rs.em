...
has_string = False
for _subfolder, msg_spec in msg_specs:
...
impl @(type_name) {
  fn get_native_message(&self) -> uintptr_t {
    return unsafe { @(c_function_prefix)_get_native_message(
@[for member in msg_spec.structure.members]@
@[    if isinstance(member.type, Array)]@
@[    elif isinstance(member.type, AbstractGenericString)]@
...
@[end for]@
  ...
  fn destroy_native_message(&self, message_handle: uintptr_t) -> () {
    unsafe {
      @(c_function_prefix)_destroy_native_message(message_handle);
    }
  }
...
@[    if isinstance(member.type, Array)]@
@[    elif isinstance(member.type, AbstractGenericString)]@
      let ptr = @(c_function_prefix)_@(member.name)_read_handle(_message_handle);
...
impl rclrs_msg_utilities::traits::MessageDefinition<@(type_name)> for @(type_name) {
  fn get_type_support() -> uintptr_t {
    return unsafe { @(c_function_prefix)_get_type_support() };
  }
